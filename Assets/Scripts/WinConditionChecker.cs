...
public class WinConditionChecker : Singleton<WinConditionChecker>
{
    ...
    private void Update()
    {
        if (!Player.Instance.PuzzleRunning)
        {
            ...
            {
                piece.Image.up = Vector3.RotateTowards(piece.Image.up, PuzzleManager.Instance.CurrentlyActivePieces[0].Image.up, Mathf.Deg2Rad * adjustmentSpeed * Time.deltaTime, 0.0f);
            }
            return;
        }
        foreach (var piece in PuzzleManager.Instance.CurrentlyActivePieces)
